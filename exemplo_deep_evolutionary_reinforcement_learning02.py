...
left_player, right_player, action, inputs, inputs_length = 0, 0, 0, [], 0
        ...
        if (hit_ball.xcor() >= 360 and hit_ball.xcor() <= 370):
            if len(inputs) > 0:
                if len(inputs) > inputs_length:
                    ...
                    deep_evolutionary_reinforcement_learning.fit(inputs=inputs)
                    inputs_length = len(inputs)
                # prediction for new action
                try: action = deep_evolutionary_reinforcement_learning.predict(predictions=1)[0][0]
                except: pass
            else: action = random
