...
def move_ball():
    ...
    action = speed_x
    if ball_y2 == 400:
        state, action = grab(), randrange(-100, 100)
        state = state.resize((300, 200))
        # save last state as image
        state.save('state.png')
        # add last state action
        deep_q_learning.addStateAction(state='state.png', action=action)
        # prediction for new action
        action = deep_q_learning.predict(state='state.png')
