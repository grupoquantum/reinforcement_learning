...
while True:
    try:
        ...
        if (hit_ball.xcor() >= 360 and hit_ball.xcor() <= 370):
            ...
            try:
                predictions += q_learning.predict()
                predictions = list(dict.fromkeys(predictions))
            except: pass
            # save the training table
            q_learning.saveQTable()
            if len(predictions) <= 0:
                # add new action
                q_learning.addActions(actions=[hit_ball.ycor()])
                # adds action quality to state
                q_learning.addStateQuality(action=hit_ball.ycor(), quality=right_player)
        ...
        if hit_ball.xcor() > 500: 
            ...
            q_learning.addActions(actions=[hit_ball.ycor()])
            # adds action quality to state
            q_learning.addStateQuality(action=hit_ball.ycor(), quality=-1*(left_player))
        # human error      
        if hit_ball.xcor() < -500: 
            hit_ball.goto(0, 0) 
            hit_ball.dy *= -1
