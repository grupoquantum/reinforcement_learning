    ...
    try:
        ...
        if (hit_ball.xcor() >= 360 and hit_ball.xcor() <= 370):
            ...
            if len(predictions) <= 0:
                # add new action
                sarsa.addActions(actions=[hit_ball.ycor()])
                # adds action quality to state
                sarsa.addStateQuality(action=hit_ball.ycor(), quality=right_player)
        ...
        if hit_ball.xcor() > 500: 
            ...
            sarsa.addActions(actions=[hit_ball.ycor()])
            # adds action quality to state
            sarsa.addStateQuality(action=hit_ball.ycor(), quality=-1*(left_player))
        # human error      
        if hit_ball.xcor() < -500: 
            hit_ball.goto(0, 0) 
            hit_ball.dy *= -1
